...
print("Importing MCMC Module: mcmc_sampling_original.py")
...
class Sampler(_ABC):
    def __init__(self, configs={}):
        ...
        if not isinstance(configs, dict):
            print(f"The passed configs object must be a dictionary; received {type(configs)}")
            raise TypeError
        else:
            if self.validate_configurations(configs):
                self._CONFIGURATIONS = configs
            else:
                print(f"The passed configurations dictionary includes invlid values/types")
                raise TypeError
    ...
    def _aggregate_configurations(self, configs, default_configs):
        ...
        if not isinstance(configs, dict) or not isinstance(default_configs, dict):
            print(f"Both of the two configs objects must be a dictionary; received {type(configs)}, {type(default_configs)}")
            raise TypeError

        updated_configs = default_configs.copy()
        for key, value in configs.items():
            if isinstance(key, dict):
                updated_configs[key] = self._aggregate_configurations(value, updated_configs[key])
            else:
...
class HMCSampler(Sampler):
    ...
    def __factorize_spsd_matrix(self, A, shape=None, lower=True):
        ...
        if isinstance(A, numbers.Number):
            ...
            valid_inds = np.asarray([i for i in range(n) if not (np.count_nonzero(A[i, :]) == np.count_nonzero(A[:, i]) == 0)])
            ...
            if valid_inds.size == 0:
                ...
                if not lower: std_reduced = std_reduced.T
            ...
            # Eliminate rows/columns with all zeros
            valid_inds = np.asarray([i for i in range(n) if not (A[[i], :].nnz == A[:, [i]].nnz == 0)])
            zero_inds  = np.setdiff1d(np.arange(n), valid_inds)
...
# #                 raise ValueError
        ...
        if sparse is not None:
            ...
            print("mass matrix used in HMC:", np.block([[mass_matrix_theta,mass_matrix_gamma]]))
...
#             raise TypeError
    ...
    def start_MCMC_sampling(self, sample_size, initial_state=None, randomize_step_size=False,
        ...
        site_count_positive=np.ones(current_state.size)
        count_positive=0
        ...
        for chain_ind in range(chain_length):
            ...
            # else:
            #     print("proposed_state",proposed_state)
...
#             print("proposed_momentum, proposed_state", proposed_momentum, proposed_state)
    ...
    def _mcmc_chain_diagnostic_statistics(self,
                                          proposals_repository,
...
def create_hmc_sampler(size,
    ...
    """
    mass_matrix_theta_multiplier=np.ones(size//2)
    mass_matrix_theta_multiplier[0]=mass_matrix_theta_multiplier[0]*mass_matrix_theta_scale
    mass_matrix_theta_multiplier[1]=mass_matrix_theta_multiplier[1]*mass_matrix_theta_scale
    mass_matrix_theta_multiplier[5]=mass_matrix_theta_multiplier[5]*mass_matrix_theta_scale
    mass_matrix_theta_multiplier[6]=mass_matrix_theta_multiplier[6]*mass_matrix_theta_scale
    ...
    configs = dict(
        ...
        mass_matrix_theta=mass_matrix_theta*mass_matrix_theta_multiplier,
    )
